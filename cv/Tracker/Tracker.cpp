...
public:
	...
	virtual bool Evaluate(double const *const *parameters,
		...
		Eigen::Vector3d pi = viframe_i->getPose() * p;
		if (pi(2) < 0.0001)
			return false;

		Eigen::Vector3d pj = T_SB * (R_ji * pi + trans_ji);
		if (pj(2) < 0.0001)
			return false;
		...
		double err = viframe_i->getCVFrame()->getIntensityBilinear(px(0), px(1), ft->level)
		...
		*residuals = w * err;
		...
		if (jacobians && jacobians[0]) {
			...
			Jac(0, 2) = -Ix * cam->cx() * pj(0) / pj(2) / pj(2) + Iy * cam->cy() * pj(1) / pj(2) / pj(2);
